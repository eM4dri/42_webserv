        ...
        for(int i = 0; i < nfds; i++) {
             ...
             if ((ep_event[i].events & EPOLLIN) == EPOLLIN)
             {
                ...
                { // request for new connection
                    ...
                    int fd_new;// Newly accept()ed socket descriptor
                    if ((fd_new = accept(listener, (struct sockaddr *)&remoteaddr, &addrlen)) == -1)
                        perror("accept");
                    // add fd_new to epoll
                    ep_event[i].events = EPOLLIN;
                    ep_event[i].data.fd = fd_new;
                    if (epoll_ctl(efd, EPOLL_CTL_ADD, fd_new, &ep_event[i]) == -1)
                        perror("epoll_ctl");
                ...
                else // data from an existing connection, receive it
                {
                    ...
                    int nbytes = recv(ep_event[i].data.fd, buf, sizeof buf, 0);

                    int sender_fd = ep_event[i].data.fd;
                    ...
                    if (nbytes <= 0) {
                        ...
                        if (close(ep_event[i].data.fd) == -1)
                            perror("close");
                        myfds.erase(sender_fd);

                    } else {
                        // We got some good data from a client
                        for (std::set<int>::iterator it = myfds.begin() ; it != myfds.end(); ++it){
